...
function FollowupLeadSub() {
    // const UserF = './img/user-5.jpg'

    const [dashboardData, setDashboardData] = useState({ Leads: { timeDurationsFollowLeads: [], contactsDataFollowLeads: [], barChartFirstFollowLeads: [] } });
    ...
    const { timeDurationsFollowLeads = [], contactsDataFollowLeads = [], barChartFirstFollowLeads = [] } = dashboardData;
        ...
        <>
            ...
            <div className="todolist-all-area ">
                <div className="todolist-first-area">
                    ...
                    {contactsDataFollowLeads.map((contact) => (
                        <UserContactInfo
                            key={contact.id}
                ...
                <div className="todolist-second-area">
                    <div className="todolist-links">
                        ...
                        >
                            <div className="text-center">
                                <div className="text-center">
                                    {contactId !== null && (
                                        <UserProfile
                                            key={contactsDataFollowLeads[contactId].id}
                                            UserImgs={contactsDataFollowLeads[contactId].UserImgs}
                                            userName={contactsDataFollowLeads[contactId].userName}
                                            mobileNumber={contactsDataFollowLeads[contactId].mobileNumber}
                                            assignDaysText="Last assign was"
                                            redTextDays="0 Days"
                                            assignTimeText="Ago, Total call duration"
                                            redTime="0 minutes"
                                            nextFollowText="Next follow-up:"
                                            dateTime={contactsDataFollowLeads[contactId].dateTime}
                                            byUser={contactsDataFollowLeads[contactId].byUser}
                                        />
                                    )}
                                </div>
                        ...
                        <div className="all-contact-content" style={{ display: isContentVisible ? 'block' : 'none' }}>
                            ...
                            <section className="text-gray-600 ">
                                <div className="p-4  mx-auto bg-[#f3f4f6] app-header px-7 py-2 ">
                                    <div className="flex flex-wrap -m-4 text-center">
                                        {timeDurationsFollowLeads.map((timeDuration, index) => (
                                            <TimeDuration
                            ...
                            {barChartFirstFollowLeads.length > 0 && (
                                <div>
                                    < BarCharts
                                        id={barChartFirstFollowLeads[0].id}
                                        barChartFirstValue={barChartFirstFollowLeads[0].barChartFirstValue}
                                        barChartSecondValue={barChartFirstFollowLeads[0].barChartSecondValue}
                                        barChartThirdValue={barChartFirstFollowLeads[0].barChartThirdValue}
                                    />
                                </div>
                            )}
                        </div>
...
export default FollowupLeadSub;