...
import { faVectorSquare, faUser, faPhone, faTag, faEnvelope, faSearch, faHistory,  faInfo, faRightLeft, faCalendarCheck, faFlag, faMap, faLocationDot, faSignsPost, faMapPin, faBank, faCode, faVenus, faCalendarDays, faCalendarPlus, faPhoneVolume, faFolder, faClock, faCheckToSlot, faSitemap, faGlobe, faCircleQuestion, faCommentDots, faSquarePhone } from '@fortawesome/free-solid-svg-icons';
...
function OpenLeadSub() {

    const [dashboardData, setDashboardData] = useState({ Leads: { timeDurationsOpenLeads: [], contactsDataOpenLeads: [], barChartFirstOpenLeads: [] } });
    ...
    const { timeDurationsOpenLeads = [], contactsDataOpenLeads = [], barChartFirstOpenLeads = [] } = dashboardData;
        ...
        <>
            ...
            <div className="todolist-all-area ">
                <div className="todolist-first-area">
                    <OrderShortBtn
                        todoHeading="Open Leads"
                    />

                    {contactsDataOpenLeads.map((contact) => (
                        <UserContactInfo
                            key={contact.id}
                        ...
                        >
                            <div className="text-center">
                                <div className="text-center">
                                    {contactId !== null && (
                                        <UserProfile
                                            key={contactsDataOpenLeads[contactId].id}
                                            UserImgs={contactsDataOpenLeads[contactId].UserImgs}
                                            userName={contactsDataOpenLeads[contactId].userName}
                                            mobileNumber={contactsDataOpenLeads[contactId].mobileNumber}
                                            assignDaysText="Last assign was"
                                            redTextDays="0 Days"
                                            assignTimeText="Ago, Total call duration"
                                            redTime="0 minutes"
                                            nextFollowText="Next follow-up:"
                                            dateTime={contactsDataOpenLeads[contactId].dateTime}
                                            byUser={contactsDataOpenLeads[contactId].byUser}
                                        />
                                    )}
                                </div>
                                ...
                                <div className="text-logo-area flex gap-4 pb-4 justify-center">
                        ...
                        <div className="all-contact-content" style={{ display: isContentVisible ? 'block' : 'none' }}>
                            <div className="search-bar-area">
                                <div className="seacrch-text">
                                    ...
                                    <h2>All Leads</h2>
                                    <p className="text-sm relative top-neg-8">Lead management is the process of capturing leads, tracking their activities, complete requirement, and qualifying and convert into opportunity for sales execoutives. </p>
                                </div>
                                <form>
                            ...
                            <section className="text-gray-600 ">
                                <div className="p-4  mx-auto bg-[#f3f4f6] app-header px-7 py-2 ">
                                    <div className="flex flex-wrap -m-4 text-center">
                                        {timeDurationsOpenLeads.map((timeDuration, index) => (
                                            <TimeDuration
                            ...
                            {barChartFirstOpenLeads.length > 0 && (
                                <div>
                                    < BarCharts
                                        id={barChartFirstOpenLeads[0].id}
                                        barChartFirstValue={barChartFirstOpenLeads[0].barChartFirstValue}
                                        barChartSecondValue={barChartFirstOpenLeads[0].barChartSecondValue}
                                        barChartThirdValue={barChartFirstOpenLeads[0].barChartThirdValue}
