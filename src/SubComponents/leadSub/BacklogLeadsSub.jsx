...
function BacklogLeadsSub() {

    const [dashboardData, setDashboardData] = useState({ Leads: { timeDurationsBacklogLeads: [], contactsDataBacklogLeads: [], barChartFirstBacklogLeads: [] } });
    ...
    const { timeDurationsBacklogLeads = [], contactsDataBacklogLeads = [], barChartFirstBacklogLeads = [] } = dashboardData;
        ...
        <>
            ...
            <div className="todolist-all-area ">
                <div className="todolist-first-area">
                    ...
                    />
                    {contactsDataBacklogLeads.map((contact) => (
                        <UserContactInfo
                            key={contact.id}
                ...
                <div className="todolist-second-area">
                    <div className="todolist-links">
                        ...
                        >
                            <div className="text-center">
                                <div className="text-center">
                                    {contactId !== null && (
                                        <UserProfile
                                            key={contactsDataBacklogLeads[contactId].id}
                                            UserImgs={contactsDataBacklogLeads[contactId].UserImgs}
                                            userName={contactsDataBacklogLeads[contactId].userName}
                                            mobileNumber={contactsDataBacklogLeads[contactId].mobileNumber}
                                            assignDaysText="Last assign was"
                                            redTextDays="0 Days"
                                            assignTimeText="Ago, Total call duration"
                                            redTime="0 minutes"
                                            nextFollowText="Next follow-up:"
                                            dateTime={contactsDataBacklogLeads[contactId].dateTime}
                                            byUser={contactsDataBacklogLeads[contactId].byUser}
                                        />
                                    )}
                                </div>
                        ...
                        <div className="all-contact-content" style={{ display: isContentVisible ? 'block' : 'none' }}>
                            ...
                            <section className="text-gray-600 ">
                                <div className="p-4  mx-auto bg-[#f3f4f6] app-header px-7 py-2 ">
                                    <div className="flex flex-wrap -m-4 text-center">
                                        {timeDurationsBacklogLeads.map((timeDuration, index) => (
                                            <TimeDuration
                            ...
                            {barChartFirstBacklogLeads.length > 0 && (
                                <div>
                                    < BarCharts
                                        id={barChartFirstBacklogLeads[0].id}
                                        barChartFirstValue={barChartFirstBacklogLeads[0].barChartFirstValue}
                                        barChartSecondValue={barChartFirstBacklogLeads[0].barChartSecondValue}
                                        barChartThirdValue={barChartFirstBacklogLeads[0].barChartThirdValue}
...
export default BacklogLeadsSub;